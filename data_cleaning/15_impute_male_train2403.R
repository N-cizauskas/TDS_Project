...
# step 2: data import
train_males <- readRDS("/rds/general/project/hda-22-23/live/TDS/Group8_Working/analysis/DATA/final_male_train_2403.rds")

# step 3: drop those with >30 missingness
# finding % missingness
male_miss <- sort((colMeans(is.na(train_males)))*100)
...
cleaned_male_data <- train_males[!(names(train_males) %in% drops)]
imputed_male <-  mice(cleaned_male_data, method="cart")
full_male <- complete(imputed_male)
print('ending imputation')

# step 6: export
saveRDS(full_male, file = "male_train_data_2503.rds")
print('saved')
...
# step 2: data import
train_females <- readRDS("/rds/general/project/hda-22-23/live/TDS/Group8_Working/analysis/DATA/toimpute_femaletrain.rds")

# step 3: drop those with >30 missingness
# finding % missingness
female_miss <- sort((colMeans(is.na(train_females)))*100)
...
cleaned_female_data <- train_females[!(names(train_females) %in% drops)]
imputed_female <-  mice(cleaned_female_data, method="cart")
full_female <- complete(imputed_female)
print('ending imputation')

# step 6: export
saveRDS(full_female, file = "imputed_female_train_2403.rds")
print('saved')